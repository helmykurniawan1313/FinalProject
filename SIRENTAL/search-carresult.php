...
<?php 
$merk=$_POST['merk'];
$sql = "select tblvehicles.*, merk.* from tblvehicles JOIN merk ON tblvehicles.merk=merk.id WHERE merk.merk='$merk'";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=$query->rowCount();
?>
<p><span><?php echo htmlentities($cnt);?> Hasil Filter</span></p>
		  </div>
		</div>
<?php 
$sql = "select tblvehicles.* , merk.* from tblvehicles JOIN merk ON tblvehicles.merk=merk.id WHERE merk.merk='$merk'";
...
<div class="product-listing-m gray-bg">
	<div class="product-listing-img"><img src="img/upload/<?php echo htmlentities($result->Vimage1);?>" class="img-responsive" alt="Image" /> </a> 
		...
		<div class="product-listing-content">
         <h5><a href="vehical-details.php?vhid=<?php echo htmlentities($result->id);?>"><?php echo htmlentities($result->merk);?> , <?php echo htmlentities($result->VehiclesTitle);?></a></h5>
         <p class="list-price">Rp. <?php echo htmlentities($result->harga);?> Per Hari</p>
       <ul>
         <li><i class="fa fa-user" aria-hidden="true"></i><?php echo htmlentities($result->kapasitas);?> Kursi</li>
         <li><i class="fa fa-calendar" aria-hidden="true"></i><?php echo htmlentities($result->transmis);?> </li>
...
<div class="sidebar_widget">
	<div class="widget_heading">
	<h5> Terakhir Kali Ditambahkan</h5>
	</div>
    <div class="recent_addedcars">
    <ul>
<?php 
		$sql = "SELECT tblvehicles.*,merk.merk,merk.id as bid  from tblvehicles join merk on merk.id=tblvehicles.merk order by id desc limit 4";
		$query = $dbh -> prepare($sql);
		$query->execute();
		$results=$query->fetchAll(PDO::FETCH_OBJ);
		$cnt=1;
